...
-- Piece rendering. Pieces are 16x16 pixel sprites drawn as horizontal runs of
-- fillRect -- see the generated SPRITES block below for why runs and not
-- gc:drawImage. Each is a solid silhouette in its side's ink plus a
-- one-pixel rim in the opposite one, because a white piece on a light square
-- and a black piece on a dark square are both nearly invisible without it.
--
...
-- One piece, rim then silhouette, at an arbitrary spot. Used by the promotion
-- chooser, which shows the four pieces exactly as they will look on the board.

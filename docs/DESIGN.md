...
The long-form version of this repo's README: what each game does, why it is
built the way it is, and what the calculator's constraints forced. The
[README](../README.md) is the short, player-facing version.
...
![Title screen](screenshots/flappy-title.png)
![In play](screenshots/flappy-playing.png)
...
![Title screen](screenshots/2048-title.png)
![In play](screenshots/2048-playing.png)
...
![Mid-slide](screenshots/2048-sliding.png)
...
![Title screen](screenshots/connect4-title.png)
![In play](screenshots/connect4-playing.png)
...
![The bot thinking](screenshots/connect4-thinking.png)
![Game over](screenshots/connect4-gameover.png)

## Chess

![The menu](screenshots/chess-menu.png)
![A piece picked up](screenshots/chess-selected.png)
...
![The bot thinking](screenshots/chess-thinking.png)
![Checkmate](screenshots/chess-checkmate.png)
...
![The promotion chooser](screenshots/chess-promotion.png)
...
![Title screen](screenshots/wordle-title.png)
![In play](screenshots/wordle-playing.png)
...
![A win](screenshots/wordle-win.png)
![A loss](screenshots/wordle-loss.png)
...
![The deal](screenshots/klondike-deal.png)
![A run picked up](screenshots/klondike-selected.png)
...
![Foundations filling](screenshots/klondike-playing.png)
![A game won](screenshots/klondike-win.png)
...
![Title screen](screenshots/slide-title.png)
![In play](screenshots/slide-playing.png)
...
![Mid-slide](screenshots/slide-sliding.png)
![The 5x5](screenshots/slide-big.png)
...
![Searching](screenshots/slide-hinting.png)
![Solved](screenshots/slide-solved.png)
...
![Title screen](screenshots/fruits-title.png)
![In play](screenshots/fruits-playing.png)
...
`H` lights a swap that works, for 20 points. There is no automatic version:
the game briefly nudged a hint at anyone who sat still for a few seconds, and
being told the answer you had not asked for turns out to be worse than being
stuck.
...
![Power fruit, with the level bar filling](screenshots/fruits-playing.png)
...
on and off**. `tools/fruitart.py` generates the outline from the sprite's own
alpha, in an 18x18 frame because the outline of a sprite that reaches the edge
of its box falls outside that box.

Two earlier attempts are worth recording, because each was worse for a
different reason. A ring around the cell is a box drawn *near* a fruit rather
than anything to do with the fruit, and it competes with the cursor ring, which
lives at the cell edge too. Swelling and shrinking the sprite looked better but
cost far more than it looks: `gc:drawImage` cannot scale, so a fruit at any
size but its native one leaves the one-call image path and arrives as about 73
rects. Flashing keeps every fruit a single blit and adds 23 rects for the
outline, on half the frames.

The outline is the *one* thing besides fruit ever filled inside a cell. The
frame reader knows that exact colour and treats it as chrome; every other stray
colour in a cell still fails the test that guards the rule. It also means a
single frame sees either every outline or none, so the reader samples a whole
flash cycle when asked which cells hold a power fruit — reading one frame is a
coin toss, and a test built on that fails intermittently rather than honestly.

Animating a *settled* board is the one thing here that costs what a still board
never used to, so it is gated twice: it repaints only when the flash actually
turns over, and only while there is a power fruit on the board. With none, a
settled board still repaints exactly never, and a test asserts it.

Two things in the harness changed along the way, and both were improvements.
`tests/fruits/frame.lua` used to decide the board had settled by watching the
repaint requests dry up, which a board that animates forever never does; it now
watches the *fruit* stop moving, which is what "settled" always meant. And it
keys colours at the handheld's own depth of five bits a channel rather than
eight — a sprite that has been through `TI.Image` comes back 15-bit while the
same sprite drawn as rects carries the original palette, so at eight bits one
fruit could read as two.

### The panel that only broke on the calculator

The side panel laid its rows out with heights written down as numbers. They
were right for DejaVu, which is what the mock measures with, and wrong for the
Nspire's own font, which is taller — so on real hardware the rule ran through
the word `SCORE` and the level bar sat on top of the word `LEVEL`. Nothing in
the preview could see it, because the preview measures with the same font the
layout was tuned against.

Every vertical position now comes from `gc:getStringHeight`, the level bar has
its own line rather than being tucked under the row above, and rows that would
not fit are dropped from the bottom instead of piling up. The score picks its
own font size from the space actually left after the rows below it are
budgeted for.

`tests/nspire_stub.lua` gained a `metricsScale` knob for this: it inflates both
text measurements so a layout can be tested against a font *bigger* than the
preview's. The panel is now rendered at up to one and a half times DejaVu's
metrics and asserted not to collide with itself — which is the test that would
have caught this before it reached a calculator.
...
![Mid-cascade](screenshots/fruits-cascade.png)
...
![A fruit picked up, with the hint lit](screenshots/fruits-selected.png)
...
![No moves left](screenshots/fruits-gameover.png)
